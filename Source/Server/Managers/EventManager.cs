...
namespace GameServer
{
    public static class EventManager
    {
        ...
        {
            if (!SettlementManager.CheckIfTileIsInUse(eventDetailsJSON.toTile)) ResponseShortcutManager.SendIllegalPacket(client, $"Player {client.username} attmepted to send an event to settlement at tile {eventDetailsJSON.toTile}, but it has no settlement");
            else
            {

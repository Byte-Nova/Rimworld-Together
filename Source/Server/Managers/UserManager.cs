...
namespace GameServer
{
    public static class UserManager
    {
        ...
        {
            bool isValid = true;
            if (string.IsNullOrWhiteSpace(data.username)) isValid = false;
            if (string.IsNullOrWhiteSpace(data.password)) isValid = false;
            if (data.username.Any(Char.IsWhiteSpace)) isValid = false;
            if (data.username.Length > 32) isValid = false;
            if (data.password.Length > 64) isValid = false;
